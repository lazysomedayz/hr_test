...
export function EmployeeForm({
  ...
  useEffect(() => {
    ...
    void api
      ...
      .catch((reason) =>
        setError(
          reason instanceof Error ? reason.message : "โหลดรหัสพนักงานไม่สำเร็จ",
        ),
      )
  ...
  return (
    <>
      <Card className="form-card">
        <CardHeader>
          <CardTitle>
            {value === "new" ? "เพิ่มพนักงาน" : "แก้ไขพนักงาน"}
          </CardTitle>
          <CardDescription>
            ข้อมูลบุคคล การทำงาน และช่องทางติดต่อ
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form ref={formRef} onSubmit={requestSave} className="grid gap-5">
            <FormSection
              ...
              description="ข้อมูลสำหรับระบุตัวพนักงาน"
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="รหัสพนักงาน">
                  <Input
                    ...
                    aria-readonly="true"
                    value={
                      codeLoading ? "กำลังสร้างรหัส..." : form.employee_code
                    }
                    className="bg-muted font-mono"
                  />
                  {value === "new" && (
                    <p className="mt-1 text-xs text-muted-foreground">
                      ระบบจะยืนยันเลขสุดท้ายอีกครั้งตอนบันทึก
                      เพื่อป้องกันรหัสซ้ำเมื่อมีการสร้างพร้อมกัน
                    </p>
                  )}
                </Field>
                ...
                <Field label="เพศ" required error={fieldErrors.gender}>
                  <Select
                    required
                    value={form.gender}
                    onValueChange={(selected) => set("gender", selected)}
                  >
                    <SelectTrigger
                      className="w-full"
                ...
                <Field
                  ...
                  errorId="error-date-of-birth"
                >
                  <BuddhistDatePicker
                    value={form.date_of_birth}
                    max={fifteenYearsAgo}
                    invalid={Boolean(fieldErrors.date_of_birth)}
                    describedBy="error-date-of-birth"
            ...
            <FormSection
              ...
              description="แผนก ตำแหน่ง และวันเริ่มงาน"
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="แผนก" required error={fieldErrors.department_id}>
                  <Select
                    ...
                    onValueChange={(selected) =>
                      set("department_id", Number(selected))
                    }
                  >
                    <SelectTrigger
                      className="w-full"
                    ...
                    <SelectContent>
                      {departments
                        .filter(
                          (item) =>
                            item.is_active || item.id === form.department_id,
                        )
                        .map((item) => (
                          <SelectItem key={item.id} value={String(item.id)}>
                            {item.name}
                            {item.is_default ? " (ค่าเริ่มต้น)" : ""}
                          </SelectItem>
                        ))}
                    </SelectContent>
                  </Select>
                ...
                <Field
                  label="วันที่เริ่มงาน (พ.ศ.)"
                  required
                  error={fieldErrors.date_joined}
                  errorId="error-date-joined"
                >
                  <BuddhistDatePicker
                    value={form.date_joined}
                    max={today}
                    invalid={Boolean(fieldErrors.date_joined)}
                    describedBy="error-date-joined"
                    onChange={(date) => set("date_joined", date)}
                  />
                </Field>
            ...
            <FormSection
              icon={<Contact />}
              title="ช่องทางติดต่อ"
              description="ข้อมูลที่ใช้ติดต่อพนักงาน"
            >
              <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="form-page-actions">
              ...
              <Button
                type="submit"
                disabled={
                  departments.length === 0 || codeLoading || !form.employee_code
                }
              >
                ตรวจสอบและบันทึก
              </Button>
      ...
      <ConfirmActionDialog
        ...
        title={
          value === "new" ? "ยืนยันการเพิ่มพนักงาน" : "ยืนยันการแก้ไขพนักงาน"
        }
