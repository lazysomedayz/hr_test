...
export default function DepartmentsPage() {
  ...
  const requestStatus = (department: Department) =>
    setPendingAction({
  ...
  const requestMove = (department: Department) =>
    setPendingAction({
  ...
  return (
    ...
    >
      {loadError ? (
        ...
        <EmptyState
          text="ยังไม่มีแผนก"
          description="สร้างแผนกแรกก่อนเพิ่มพนักงาน"
          action={() => navigate("/departments/new")}
          actionText="เพิ่มแผนก"
        />
      ) : (
            ...
            <Card className="department-card" key={department.id}>
              <CardContent className="p-0">
                ...
                <div className="card-actions">
                  <IconAction
                    label={`แก้ไขแผนก ${department.name}`}
                    icon={<Edit3 />}
                    onClick={() =>
                      navigate(`/departments/${department.id}/edit`)
                    }
                  />
                  {!department.is_default && department.employee_count > 0 && (
                    <IconAction
                      label={`ย้ายพนักงานจาก ${department.name} ไปแผนกกลาง`}
                      icon={<ArrowLeftRight />}
                      onClick={() => requestMove(department)}
                    />
                  )}
                  {!department.is_default && (
                    <IconAction
                      destructive={department.is_active}
                      label={`${department.is_active ? "ปิด" : "เปิด"}ใช้งานแผนก ${department.name}`}
                      icon={<Power />}
                      onClick={() => requestStatus(department)}
                    />
                  )}
                  {!department.is_default && (
                    <IconAction
                      destructive
                      label={`ลบแผนก ${department.name}`}
                      icon={<Trash2 />}
      ...
      {deleteTarget && (
        <DeleteDialog
          open
          title={`ลบแผนก “${deleteTarget.name}”?`}
          description="แผนกจะถูกนำออกจากรายการและไม่สามารถนำไปเลือกให้พนักงานใหม่ได้"
          blockedReason={
            deleteTarget.employee_count > 0
              ? `แผนกนี้มีพนักงาน ${deleteTarget.employee_count} คน กรุณาย้ายพนักงานไปแผนกกลางก่อน`
              : undefined
          }
...
function DepartmentSkeleton() {
  return (
    <div
      ...
      role="status"
      aria-label="กำลังโหลดข้อมูลแผนก"
    >
